...
class Alaug(object):
    ...
    def aug(self, data):
        ...
        aug_kwargs = {"image": img}
        if self.keypoint_params is not None:
            aug_kwargs["keypoints"] = keypoints_val if keypoints_val is not None else []
        if masks is not None:
            aug_kwargs["mask"] = masks
        if self.bbox_params is not None:
            aug_kwargs["bboxes"] = bboxes if bboxes is not None else []
            aug_kwargs["bbox_labels"] = bbox_labels if bbox_labels is not None else []
        aug = self.__augmentor(**aug_kwargs)
        data['img'] = aug['image']
